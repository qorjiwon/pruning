...
const ImageUpload: React.FC<Props> = ({ setMode }) => {
  ...
  const handleFileDrop = async (file: File) => {
    try {
      ...
      toast({
        title: "오류 발생",
        description: "이미지 업로드 또는 분석 중 문제가 발생했습니다.",
        variant: "destructive",
      });
    }
  };
  ...
  const renderComment = () => {
    return comment.split('\n').map((line, index) => {
      const isMainNumbered = /^\d+\./.test(line);
      return (
        <p key={index} className={isMainNumbered ? "mt-4" : ""}>
          {line}
        </p>
      );
    });
        ...
        {!originalImage ? (
          ...
          <>
            <div className="flex flex-col md:flex-row gap-6">
              {/* 좌측 영역 */}
              <div className="flex-1 flex-col bg-[#E3DFB278] rounded-[30px] px-4 pb-6 h-[550px] flex items-center justify-center overflow-hidden">
                <div className="flex items-center justify-between mb-4 w-full px-1 pt-6">
                  <div className="px-12 text-base leading-7">
                    <p>
              ...
              <div className="flex-1 bg-[#F5F5D5]">
                <div className="bg-[#E3DFB278] flex items-start justify-center rounded-[30px] p-6 h-[550px] text-xl leading-[22px] overflow-y-auto">
                  <div className="w-full">{renderComment()}</div>
                </div>
              </div>
            </div>
